...
> **Historical note**: At the time of v0.1.9, `main` was the default development branch. Since then, `dev` has become the default (integration) branch and `main` is reserved for releases only. The kangaroo submodule continues to use its own `main`/`release` split independently.
