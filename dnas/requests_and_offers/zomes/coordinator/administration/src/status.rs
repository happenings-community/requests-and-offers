...
pub fn create_status(input: EntityActionHash) -> ExternResult<Record> {
  ...
  if !links.is_empty() {
    return Err(
      StatusError::AlreadyHasStatus("This status already has a Status".to_string()).into(),
    );
  }

  let status_hash = create_entry(&EntryTypes::Status(Status::pending()))?;
  let record = get(status_hash.clone(), GetOptions::default())?.ok_or(
    CommonError::EntryNotFound("Could not find the newly created profile's Status".to_string()),
  )?;
...
pub fn get_latest_status_record(original_action_hash: ActionHash) -> ExternResult<Option<Record>> {
  ...
  let latest_status_hash = match latest_link {
    Some(link) => link
      ...
      .clone()
      .into_action_hash()
      .ok_or(CommonError::EntryNotFound("Status not found".to_string()))?,
...
pub fn get_latest_status_record_for_entity(
  ...
  if !links.is_empty() {
    get_latest_status_record(links[0].clone().target.into_action_hash().ok_or(
      CommonError::ActionHashNotFound("Status not found".to_string()),
    )?)
  } else {
    Ok(None)
  }
...
pub fn get_latest_status_for_entity(input: EntityActionHash) -> ExternResult<Option<Status>> {
  ...
  let latest_status: Option<Status> = if !links.is_empty() {
    get_latest_status(links[0].target.clone().into_action_hash().ok_or(
      CommonError::ActionHashNotFound("Status not found".to_string()),
    )?)?
  } else {
    None
...
pub fn update_entity_status(input: UpdateEntityActionHash) -> ExternResult<Record> {
  ...
  let record = get(action_hash.clone(), GetOptions::default())?.ok_or(
    CommonError::EntryNotFound("Could not find the newly updated Status".to_string()),
  )?;
...
pub fn suspend_entity_temporarily(input: SuspendEntityInput) -> ExternResult<bool> {
  let duration_in_days = match input.duration_in_days {
    ...
    None => {
      return Err(
        CommonError::InvalidEntryData("Duration in days must be provided".to_string()).into(),
      )
    }
  };
...
pub fn unsuspend_entity_if_time_passed(input: UpdateInput) -> ExternResult<bool> {
  ...
  let link = match link.first() {
    ...
    None => {
      return Err(
        CommonError::EntryNotFound("Could not find the entity's Status link".to_string()).into(),
      )
    }
  };

  let status_action_hash =
    link
      .clone()
      .target
      .into_action_hash()
  ...
  let mut status = get_latest_status(status_action_hash)?.ok_or(CommonError::EntryNotFound(
