...
pub fn get_all_users(_: ()) -> ExternResult<Vec<Link>> {
  ...
  let link_type_filter = LinkTypes::AllUsers
    .try_into_filter()
...
pub fn get_user_status_link(user_original_action_hash: ActionHash) -> ExternResult<Option<Link>> {
  let link_type_filter = LinkTypes::UserStatus
    ...
    .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?;
  let links = get_links(
    LinkQuery::new(user_original_action_hash.clone(), link_type_filter),
    GetStrategy::Network,
  )?;
...
pub fn get_organization_status_link(
  ...
  let link_type_filter = LinkTypes::OrganizationStatus
    .try_into_filter()
    .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?;
  let links = get_links(
    LinkQuery::new(organization_original_action_hash.clone(), link_type_filter),
    GetStrategy::Network,
  )?;
