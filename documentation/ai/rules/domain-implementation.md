...
export const useCapabilityGuard = (requiredCapability: string) =>
  Effect.gen(function* () {
    ...
    const requireCapability = hasCapability
      ? Effect.unit
      : Effect.fail(new InsufficientCapabilityError({
...
export const createServiceType = (input: CreateServiceTypeInput) =>
  Effect.gen(function* () {
    ...
    Effect.mapError(transformErrorWithContext(ErrorContexts.SERVICE_TYPE_CREATION))
  );
```
...
export const createEntityWithRecovery = <TInput, TEntity>(
...
export const withRetry = <T, E>(
  ...
  effect.pipe(
    Effect.retry(Schedule.exponential(baseDelay, 2.0).pipe(
...
export const useInputValidationGuard = <TSchema>(schema: Schema.Schema<TSchema>) =>
  (input: unknown) =>
    Effect.gen(function* () {
...
export const useBusinessRuleGuard = <TEntity>(
...
export const useStateTransitionGuard = <TStatus>(
...
export const requestStatusTransitions = {
  ...
  [RequestStatus.Published]: [RequestStatus.InProgress, RequestStatus.Cancelled],
...
export const makeAvatarService = Effect.gen(function* () {
  ...
  const uploadAvatar = (userId: string, imageFile: File) =>
    Effect.gen(function* () {
      ...
      // Upload to storage
      const avatarUrl = yield* storageService.uploadFile(avatarPath, optimizedImage);
      ...
      Effect.mapError(error => new AvatarError({
...
// Tag normalization
export const normalizeTags = (tags: string[]): Effect.Effect<string[], ValidationError> =>
  Effect.gen(function* () {
