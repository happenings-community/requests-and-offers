...
**CI runs the suite under `cargo-nextest`, which is a workaround for a real leak.** The suite leaks roughly 620MB per minute across the tests inside a single `cargo test` binary, so on a 16GB runner the two largest binaries exhaust memory and GitHub kills the runner before they finish. This was measured, not guessed: the job samples free memory every 30 seconds and the curve falls in a straight line from 13.6GB to 71MB while disk stays flat at 72G. Nextest runs each test in its own process, which hands that memory back at every test boundary. The leak itself is [#265](https://github.com/happenings-community/requests-and-offers/issues/265) and is still open; `cargo test` remains the local command, where 16 cores and more memory hide the problem.
