...
**CI runs each test in its own `cargo test` process, which is a workaround for a real leak.** The suite leaks roughly 620MB per minute across the tests inside a single `cargo test` binary, so on a 16GB runner the two largest binaries exhaust memory and GitHub kills the runner before they finish. This was measured, not guessed: the job samples free memory every 30 seconds and the curve falls in a straight line from 13.6GB to 71MB while disk stays flat at 72G. Running one test per process hands that memory back at every test boundary, and drops the peak to a single test's two conductors. `cargo-nextest` does this natively and was tried first, but its list step could not load `libssl.so.3` from the Nix store, so the loop uses the toolchain that already works in this shell rather than a fetched binary plus an `LD_LIBRARY_PATH` workaround. The leak itself is [#265](https://github.com/happenings-community/requests-and-offers/issues/265) and is still open; Locally a plain `cargo test` is still the right command, where 16 cores and more memory hide the problem.
