...
async function identifyDNAs(player: Player): Promise<DNAInfo[]> {
  ...
  for (let i = 0; i < player.cells.length; i++) {
    ...
    if (dnaName === "unknown") {
      ...
      try {
        ...
        if (appInfo) {
          for (const [roleName, cellInfo] of Object.entries(appInfo.cell_info)) {
            if (Array.isArray(cellInfo) && cellInfo.length > 0) {
              const cellData = cellInfo[0];
              if ('provisioned' in cellData) {
                const provisioned = cellData.provisioned;
...
test(
  "DNA ordering consistency test - identify which DNA is at which index",
  async () => {
    await runScenarioWithTwoAgents(
      async (_scenario: Scenario, alice: Player, bob: Player) => {
        ...
        aliceDNAs.forEach((dna, index) => {
          console.log(`  Cell[${index}]: ${dna.name} (zomes: ${dna.zomes.join(', ')})`);
        });
        ...
        if (alice.cells.length !== bob.cells.length) {
          console.log(`❌ ISSUE: Alice has ${alice.cells.length} cells, Bob has ${bob.cells.length} cells`);
        ...
        for (let i = 0; i < Math.min(alice.cells.length, bob.cells.length); i++) {
          ...
          if (aliceDNA.name !== bobDNA.name) {
            console.log(`❌ ISSUE: At index ${i}, Alice has '${aliceDNA.name}' but Bob has '${bobDNA.name}'`);
            orderingConsistent = false;
          } else {
            console.log(`✅ Index ${i}: Both Alice and Bob have '${aliceDNA.name}'`);
          }
        }
        ...
        const aliceRequestsAndOffersIndex = aliceDNAs.findIndex(dna => dna.name === "requests_and_offers");
        const bobRequestsAndOffersIndex = bobDNAs.findIndex(dna => dna.name === "requests_and_offers");
        ...
        if (aliceRequestsAndOffersIndex !== bobRequestsAndOffersIndex) {
          console.log(`❌ CRITICAL ISSUE: requests_and_offers DNA is at different indices!`);
          console.log(`   This explains why tests fail - they assume it's always at index 0`);
        }
        
        if (aliceRequestsAndOffersIndex !== 0 || bobRequestsAndOffersIndex !== 0) {
          console.log(`❌ ISSUE: requests_and_offers DNA is not at index 0 as expected by tests`);
        }
        ...
        if (aliceRequestsAndOffersIndex >= 0 && bobRequestsAndOffersIndex >= 0) {
          ...
          try {
            const aliceServiceTypes = await alice.cells[aliceRequestsAndOffersIndex].callZome({
              zome_name: "service_types",
              fn_name: "get_all_service_types",
              payload: null,
            });
            ...
            const bobServiceTypes = await bob.cells[bobRequestsAndOffersIndex].callZome({
              zome_name: "service_types", 
              fn_name: "get_all_service_types",
              payload: null,
            });
...
test(
  ...
  async () => {
    const results: Array<{ run: number; aliceIndex: number; bobIndex: number }> = [];
      ...
      try {
        await runScenarioWithTwoAgents(
          async (_scenario: Scenario, alice: Player, bob: Player) => {
            ...
            const aliceIndex = aliceDNAs.findIndex(dna => dna.name === "requests_and_offers");
            const bobIndex = bobDNAs.findIndex(dna => dna.name === "requests_and_offers");
            
            results.push({ run, aliceIndex, bobIndex });
            
            console.log(`Run ${run}: Alice requests_and_offers at index ${aliceIndex}, Bob at index ${bobIndex}`);
          }
        );
      } catch (error) {
    ...
    results.forEach(result => {
      ...
      console.log(`  Run ${result.run}: Alice[${result.aliceIndex}], Bob[${result.bobIndex}] - ${consistent ? '✅ Consistent' : '❌ Inconsistent'} - ${atExpectedIndex ? '✅ At expected index 0' : '❌ Not at expected index'}`);
    });
    
    // Check for inconsistencies
    const hasInconsistentOrdering = results.some(r => r.aliceIndex !== r.bobIndex);
    const hasNonZeroIndices = results.some(r => r.aliceIndex !== 0 || r.bobIndex !== 0);
    const hasVariableIndices = new Set(results.map(r => `${r.aliceIndex}-${r.bobIndex}`)).size > 1;
    
    console.log(`\nIssues detected:`);
    console.log(`  Inconsistent ordering between agents: ${hasInconsistentOrdering ? '❌ YES' : '✅ NO'}`);
    console.log(`  DNA not at expected index 0: ${hasNonZeroIndices ? '❌ YES' : '✅ NO'}`);
    console.log(`  Variable indices across runs: ${hasVariableIndices ? '❌ YES' : '✅ NO'}`);
    ...
    if (hasInconsistentOrdering || hasNonZeroIndices || hasVariableIndices) {
      ...
      console.log(`   Note: The issue might be intermittent - run this test multiple times`);
    }
  },
  {
