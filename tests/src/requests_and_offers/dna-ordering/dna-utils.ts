import { Player, CallableCell, PlayerApp } from "@holochain/tryorama";
 ...
 */
export async function getRequestsAndOffersCell(
  player: PlayerApp,
): Promise<CallableCell | null> {
 ...
 */
export async function getHREACell(
  player: PlayerApp,
): Promise<CallableCell | null> {
  for (const cell of player.cells) {
    try {
 ...
 */
export async function getRequestsAndOffersIndex(
  player: PlayerApp,
): Promise<number> {
 ...
 */
export async function getHREAIndex(player: PlayerApp): Promise<number> {
  for (let i = 0; i < player.cells.length; i++) {
    try {
      await player.cells[i].callZome({
        zome_name: "agent",
 ...
 */
export async function analyzeDNAStructure(
  player: PlayerApp,
...
export async function runScenarioWithReliableCells(
  ...
  await runScenarioWithTwoAgents(
    async (scenario, alice: PlayerApp, bob: PlayerApp) => {
      ...
      if (!aliceRequestsAndOffers || !bobRequestsAndOffers) {
        throw new Error(
          "Could not find requests_and_offers DNA cells for both players",
        );
      }
