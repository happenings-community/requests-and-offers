...
test("Simple DNA ordering identification test", async () => {
  await runScenarioWithTwoAgents(
    async (_scenario: Scenario, alice: PlayerApp, bob: PlayerApp) => {
      ...
      assert.equal(
        alice.cells.length,
        2,
        "Alice should have 2 cells (requests_and_offers + hrea)",
      );
      assert.equal(
        bob.cells.length,
        2,
        "Bob should have 2 cells (requests_and_offers + hrea)",
      );

      // Get cell IDs to see if they're in the same order
      const aliceCellIds = alice.cells.map((cell) =>
        cell.cell_id[0].toString(),
      );
      ...
      for (
        let i = 0;
        i < Math.min(aliceCellIds.length, bobCellIds.length);
        i++
      ) {
        ...
        console.log(
          `  Index ${i}: Alice(${aliceId}) vs Bob(${bobId}) - ${match ? "✅ Match" : "❌ Different"}`,
        );
      }
      ...
      console.log(`\n=== RESULTS ===`);
      console.log(
        `Alice requests_and_offers at index: ${aliceRequestsAndOffersIndex}`,
      );
      console.log(
        `Bob requests_and_offers at index: ${bobRequestsAndOffersIndex}`,
      );

      // Check for issues
      if (aliceRequestsAndOffersIndex !== bobRequestsAndOffersIndex) {
        console.log(
          `❌ ISSUE: requests_and_offers DNA is at different indices between Alice and Bob`,
        );
        console.log(
          `   This explains why tests that assume cells[0] would fail!`,
        );
      } else {
        console.log(
          `✅ requests_and_offers DNA is at the same index for both agents`,
        );
      }

      if (aliceRequestsAndOffersIndex !== 0) {
        console.log(
          `❌ ISSUE: requests_and_offers DNA is not at index 0 (it's at index ${aliceRequestsAndOffersIndex})`,
        );
        console.log(
          `   Tests that assume cells[0] is requests_and_offers will fail!`,
        );
      } else {
      ...
      if (
        ...
        console.log(
          `   Root cause: Tryorama doesn't guarantee DNA ordering in multi-DNA hApps`,
        );
        console.log(
          `   Solution: Use dynamic DNA identification instead of hardcoded indices`,
        );
      }
