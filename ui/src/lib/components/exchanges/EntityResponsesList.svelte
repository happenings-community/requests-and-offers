<script lang="ts">
  ...
  import type { UIExchangeResponse } from '$lib/schemas/exchanges.schemas';
  ...
  const { entityHash, entityType, showAsOwner = false }: Props = $props();
  ...
  $effect(() => {
    async function loadResponses() {
      try {
        isLoading = true;
        const entityResponses = await runEffect(
          exchangesStore.fetchResponsesForEntity(entityHash)(responsesSetters)
        );
            ...
            <div class="flex-1 space-y-3">
              ...
              <div class="flex items-center gap-3">
                <div class="flex items-center gap-2">
                  ...
                  >
                    {response.proposerPubkey
                      ? response.proposerPubkey.slice(0, 2).toUpperCase()
                      : 'U'}
                  </div>
                  <div>
                    <p class="text-sm font-medium">Response from</p>
                    <p class="text-xs text-surface-500">
                      {response.proposerPubkey || 'Unknown user'}
                    </p>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                  <h5 class="mb-1 text-sm font-medium">Service Details</h5>
                  <p class="text-sm text-surface-600 dark:text-surface-400">
                ...
                <div>
                  <h5 class="mb-1 text-sm font-medium">Terms</h5>
                  <p class="text-sm text-surface-600 dark:text-surface-400">
                ...
                <div>
                  <h5 class="mb-1 text-sm font-medium">Exchange Medium</h5>
                  <p class="text-sm text-surface-600 dark:text-surface-400">
                ...
                {#if response.entry.exchange_value}
                  <div>
                    <h5 class="mb-1 text-sm font-medium">Exchange Value</h5>
                    <p class="text-sm text-surface-600 dark:text-surface-400">
                ...
                {#if response.entry.delivery_timeframe}
                  <div>
                    <h5 class="mb-1 text-sm font-medium">Timeframe</h5>
                    <p class="text-sm text-surface-600 dark:text-surface-400">
                ...
                {#if response.entry.notes}
                  <div class="md:col-span-2">
                    <h5 class="mb-1 text-sm font-medium">Notes</h5>
                    <p class="text-sm text-surface-600 dark:text-surface-400">
              ...
              <div class="flex items-center justify-between text-xs text-surface-500">
                <span>
                  Submitted {response.entry.created_at
