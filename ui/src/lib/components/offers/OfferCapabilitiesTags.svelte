<script lang="ts">
  ...
  const { capabilities, serviceTypeActionHash, maxVisible = 3 }: Props = $props();
    ...
    if (serviceTypeActionHash) {
      ...
      pipe(
        ...
        E.catchAll((err) => {
          ...
          isLoadingServiceType = false;
          return E.succeed(null); // Gracefully handle error
        }),
        E.runPromise
      );
    } else {
...
<div class="flex flex-wrap items-center gap-2">
  ...
  {:else if displayItems.length === 0 && (!capabilities || capabilities.length === 0)}
