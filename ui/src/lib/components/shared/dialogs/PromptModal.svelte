  ...
  function detectCapsLock(event: KeyboardEvent, inputName: string) {
    ...
    if (event.key && event.key.length === 1) {
      ...
      if (isLetter) {
        ...
        capsLockStates[inputName] =
          (isUpperCase && !isShiftPressed) || (!isUpperCase && isShiftPressed);
      }
    }
  }
  ...
  <div class="static space-y-8">
    ...
    <form class="space-y-10" onsubmit={submitForm} bind:this={form}>
        ...
        <label class="space-y-4">
          ...
          {#if input.type === 'password'}
            <div class="relative">
              <input
                ...
                class="input bg-surface-300 pr-12 text-black dark:bg-surface-600 dark:text-white"
                name={input.name}
                value={input.value}
                min={input.min}
                max={input.max}
                onkeydown={(e) => {
                  detectCapsLock(e, input.name);
                  handleCapsLockKey(e, input.name);
                }}
              ...
              <button
                type="button"
                class="absolute right-3 top-1/2 -translate-y-1/2 text-surface-600 hover:text-surface-800 focus:outline-none dark:text-surface-400 dark:hover:text-surface-200"
              ...
              {#if capsLockStates[input.name]}
                <div
                  class="absolute -bottom-6 left-0 flex items-center gap-2 text-sm text-warning-600"
                >
          ...
          {:else}
            <input
              type={input.type}
              placeholder={input.placeholder}
              class="input bg-surface-300 text-black dark:bg-surface-600 dark:text-white"
              name={input.name}
              value={input.value}
              min={input.min}
              max={input.max}
      ...
      <div class="flex justify-center gap-4">
        <button type="submit" class="variant-filled-tertiary btn w-fit self-center">Submit</button>
        <button
          type="button"
          class="variant-filled-error btn w-fit self-center"
