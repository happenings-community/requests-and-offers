<script lang="ts">
  ...
  const hasData = $derived(
    state.serviceTypes.length > 0 || state.requests.length > 0 || state.offers.length > 0
  );
    ...
    try {
      ...
      if (showRelatedTags) {
        try {
          ...
          state.relatedTags = tagStats.filter(([tagName]) => tagName !== tag).slice(0, 10); // Top 10 related tags
...
{:else if state.error}
  <div class="alert variant-filled-error">
    ...
    <div class="alert-actions">
      <button class="variant-soft btn btn-sm" onclick={reloadData}> Try Again </button>
    </div>
  </div>
  ...
  <div class="space-y-8">
    ...
    {#if showStatistics}
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div class="card bg-tertiary-50 p-6 text-center dark:bg-tertiary-900/20">
    ...
    <div class="card bg-surface-100 p-6 text-center dark:bg-surface-800">
      ...
      {#if enableNavigation}
        <div class="mt-4 flex flex-wrap justify-center gap-2">
          <a href="/service-types" class="variant-soft-primary btn btn-sm"> All Service Types </a>
          <a href="/requests" class="variant-soft-secondary btn btn-sm"> All Requests </a>
          <a href="/offers" class="variant-soft-tertiary btn btn-sm"> All Offers </a>
        </div>
      {/if}
    </div>
  </div>
