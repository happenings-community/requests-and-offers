...
import type { UIExchangeReview, UIAgreement, UIUser, BaseComposableState } from '$lib/types/ui';
import type {
...
// Error handling
export class ExchangeFeedbackManagementError extends Data.TaggedError(
  'ExchangeFeedbackManagementError'
)<{
  message: string;
  context?: string;
  cause?: unknown;
...
export interface UseExchangeFeedbackManagement
  extends ExchangeFeedbackManagementState,
...
export function useExchangeFeedbackManagement(): UseExchangeFeedbackManagement {
    ...
    return completedAgreements.filter((agreement) => {
      ...
      const hasReviewed = allReviews.some(
        (review) =>
          ...
          encodeHashToBase64(review.agreement_hash) ===
            encodeHashToBase64(agreement.original_action_hash!) &&
    ...
    if (state.timeFilter !== 'all') {
      ...
      if (timeLimit) {
        filtered = filtered.filter(
          (review) => review.created_at && now - review.created_at <= timeLimit
        );
      }
    }
    ...
    pipe(
      E.gen(function* () {
        ...
        if (!isUserApproved(currentUser) && !isAdministrator) {
          ...
          if (status === 'pending') {
            errorMessage =
              "Your profile is pending approval. You'll be able to view exchange feedback once approved.";
        ...
        yield* E.all([exchangesStore.getAllExchangeReviews(), exchangesStore.getAllAgreements()]);
  ...
  async function initialize(): Promise<void> {
    ...
    try {
      ...
      await runEffect(
        usersStore.refreshCurrentUser().pipe(
          E.catchAll((error) => {
  ...
  async function submitMutualValidation(
    ...
    try {
      ...
      const createEffect = pipe(
        ...
        E.mapError((error) =>
          ExchangeFeedbackManagementError.fromError(error, 'submitMutualValidation')
        )
      );
      ...
      const validationError = ExchangeFeedbackManagementError.fromError(
        error,
        'submitMutualValidation'
      );
  ...
  async function submitPublicReview(
    ...
    try {
      ...
      const createEffect = pipe(
        ...
        E.mapError((error) =>
          ExchangeFeedbackManagementError.fromError(error, 'submitPublicReview')
        )
      );
  ...
  async function calculateUserStats(userHash?: ActionHash): Promise<ReviewStats> {
    try {
      ...
      const publicReviews = userReviews.filter(
        (review) => review.is_public && review.public_review?.rating
      );
      ...
      const totalRating = publicReviews.reduce(
        (sum, review) => sum + (review.public_review?.rating || 0),
        0
      );
      const averageRating = totalRating / publicReviews.length;

      const onTimeCount = publicReviews.filter(
        (review) => review.public_review?.completed_on_time
      ).length;
      const asAgreedCount = publicReviews.filter(
        (review) => review.public_review?.completed_as_agreed
      ).length;
      ...
      for (let i = 0; i <= 5; i++) {
        ratingDistribution[i] = publicReviews.filter(
          (review) => review.public_review?.rating === i
        ).length;
      }
