...
2. **Status-history pages render their empty state** — FIXED, three layers: (a) `get_all_revisions_for_entry` uses `GetStrategy::Network` instead of `Local`, so revision links authored by other agents (admin status updates) are found; (b) `get_latest_status_record` queries `StatusUpdates` links (the `AllStatuses` links are anchored from the `"{entity}.status"` path, not from a status action hash, so the old query returned nothing); (c) the store's `fetchAllUsersStatusHistory` / `fetchAllOrganizationsStatusHistory` now fetch the entity lists themselves instead of trusting in-memory state, which was empty on direct navigation to `/admin/*/status-history`. Tests: `01-administration` and `06-organizations` "status history page renders" now require rows, no empty-state fallback.
3. **Medium-of-exchange updates are invisible in the UI** — FIXED: `get_mediums_of_exchange_by_status` resolves the latest record per link; the MoE store's `fixEntityOriginalHash` corrects `original_action_hash` on Update records; the edit page loads via `getLatestMediumOfExchangeRecord`; and `updateMediumOfExchange` removes the stale row by its original hash and re-keys the updated entity by the true original hash (previously it could leave old + new duplicate rows in state on in-place updates). Test: `03-mediums-of-exchange` "admin edits the medium" now asserts the edited name appears in the admin list (and the pre-edit name, exact-matched, is gone).
