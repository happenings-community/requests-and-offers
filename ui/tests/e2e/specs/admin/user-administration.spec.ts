...
test.describe('User Administration with Real Holochain Data', () => {
  ...
  test('Admin can view all users with comprehensive information', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="user-row"]')).toHaveCount(totalUsers, { timeout: 15000 });
  ...
  test('Admin can suspend and unsuspend users', async ({ page }) => {
    ...
    await page.fill('[data-testid="suspension-reason"]', 'E2E test suspension - inappropriate behavior');
    ...
    await page.fill('[data-testid="unsuspension-reason"]', 'E2E test unsuspension - issue resolved');
    
    // Confirm unsuspension
    await page.click('[data-testid="confirm-unsuspension"]');

    // Wait for unsuspension to complete
    await expect(page.locator('text=User unsuspended successfully')).toBeVisible({ timeout: 15000 });
  ...
  test('Admin can manage user roles and permissions', async ({ page }) => {
    ...
    await page.fill('[data-testid="role-change-reason"]', `E2E test role change from ${currentRole} to ${newRole}`);
    
    // Confirm role change
    await page.click('[data-testid="confirm-role-change"]');

    // Wait for role change to complete
    await expect(page.locator('text=User role updated successfully')).toBeVisible({ timeout: 15000 });
  ...
  test('Admin can promote users to moderator status', async ({ page }) => {
    ...
    await page.fill('[data-testid="promotion-justification"]', 'E2E test promotion - user has shown excellent community engagement');
  ...
  test('Admin can bulk manage users', async ({ page }) => {
    ...
    await page.fill('[data-testid="notification-message"]', 'This is a test notification sent during E2E testing');
    
    // Send notification
    await page.click('[data-testid="send-notification"]');

    // Wait for notification to be sent
    await expect(page.locator('text=Notification sent successfully')).toBeVisible({ timeout: 15000 });
  });
});
