...
test.describe('Cross-Entity Relationship Tests with Real Holochain Data', () => {
  ...
  test('Offers correctly link to service types and display relationships', async ({ page }) => {
    ...
    if (firstOffer.serviceTypeHashes.length > 0) {
      ...
      const serviceType = seededData.serviceTypes.find(st => st.actionHash === firstServiceTypeHash);
    ...
    if (firstRequest.mediumOfExchangeHashes.length > 0) {
      ...
      if (medium) {
        ...
        if (filteredRequests.length > 0) {
          await expect(page.locator('[data-testid="request-card"]')).toHaveCount(filteredRequests.length);
        }
      }
    }
  });
  ...
  test('Organization pages correctly display member relationships and hierarchy', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('text=Connecting to Holochain...')).toBeHidden({ timeout: 10000 });
  ...
  test('Service type relationships cascade correctly through offers and requests', async ({ page }) => {
    await page.goto('/');
    await expect(page.locator('text=Connecting to Holochain...')).toBeHidden({ timeout: 10000 });
    ...
    if (relatedRequests.length > 0) {
      ...
      await expect(page.locator('[data-testid="request-card"]')).toHaveCount(relatedRequests.length);
  ...
  test('Medium of exchange relationships work across offers and requests', async ({ page }) => {
    ...
    if (requestsWithMedium.length > 0) {
      await expect(page.locator('[data-testid="request-card"]')).toHaveCount(requestsWithMedium.length);
  ...
  test('Data consistency across all entity relationships', async ({ page }) => {
    ...
    for (const offer of seededData.offers.slice(0, 3)) { // Test first 3 offers
    ...
    for (const request of seededData.requests.slice(0, 3)) { // Test first 3 requests
    ...
    for (const user of seededData.users.slice(0, 3)) { // Test first 3 users
